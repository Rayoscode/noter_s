...
import {
  getClosestElementOfCaretByRange,
  getClosestElementWithSelectorOnCaretPosition,
  getClosestElementWithSelectorOnRange,
  ...
  try {
    ...
    if (isTodoList(listItem)) {
      getSelection()?.setPosition(
        nextItem?.lastElementChild?.firstElementChild as Node
      );
    } else {
...
export function handleInsertParagraphEventOnListItem(
  range: Range,
